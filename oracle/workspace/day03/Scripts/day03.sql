...
SELECT COUNT(TEAM_ID) FROM PLAYER WHERE TEAM_ID = 'K01';
...
/*PLAYER 테이블에서 TEAM_ID가 'K01'인 선수 이름 내 이름으로 바꾸기*/
...
SELECT COUNT(HEIGHT) FROM PLAYER WHERE HEIGHT >= 180;

SELECT COUNT(*) FROM PLAYER;

/*PLAYER 테이블에서 NICKNAME이 NULL인 선수 검색*/
SELECT PLAYER_NAME, NVL(NICKNAME, '없음')  FROM PLAYER WHERE NICKNAME IS NULL;
...
SELECT PLAYER_NAME, BACK_NO "등 번호", NICKNAME "선수 별명" FROM PLAYER;
...
/*PLAYER_NAME의 영어이름은 E*/
SELECT PLAYER_NAME || '의 영어이름은' || E_PLAYER_NAME ||'이다' AS 소개 FROM PLAYER;
...
SELECT COUNT(NVL(HEIGHT, 0)) "총 선수(명)" FROM PLAYER;
...
/*PLAYER 테이블에서 몸무게가 80이상인 선수들의 평균키가 180 이상이면 포지션 검색*/
...
/*EMPLOYEES 테이블에서 JOB_ID별 평균 SALARY가 10000미만인  JOB_ID 검색, JOB_ID 알파벳 순 정렬*/
...
/*PLAYER 테이블에서 전체 평균의 키와 포지션별 평균 키*/
...
/*EMPOLYEES 테이블에서 평균 급여보다 낮은 사원들의 급여를 20% 인상*/
SELECT * FROM EMPLOYEES;

SELECT FIRST_NAME, SALARY + SALARY * 0.2 "RESULT" FROM EMPLOYEES  
WHERE SALARY <
 ...
 * 입력, 수정, 삭제의 성능을 향상시키기 위해서 JOIN을 통해 합친후 사용
 * 
 * */


/*EMP 테이블 사원번호로 DEPT테이블의 지역 검색*/
	SELECT * FROM EMP;
	SELECT * FROM DEPT;

SELECT ENAME, LOC FROM DEPT D JOIN EMP E
ON D.DEPTNO = E.DEPTNO;

/*PLAYER 테이블에서 송종국 선수가 속한 팀의 전화번호 검색하기*/
SELECT * FROM PLAYER;
SELECT * FROM TEAM;
SELECT * FROM PLAYER WHERE PLAYER_NAME = '송종국';


SELECT T.TEAM_ID , PLAYER_NAME , TEL FROM PLAYER P JOIN TEAM T 
...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE BETWEEN '2003-01-01' AND '2005-12-31'
ORDER BY HIRE_DATE;

/*JOB_TITLE 중 'Manager'라는 문자열이 포함된 직업들의 평균 연봉을 JOB_TITLE별로 검색*/
SELECT JOB_ID, JOB_TITLE, (MIN_SALARY + MAX_SALARY)/2 "avg" FROM JOBS  WHERE JOB_TITLE LIKE '%Manager';
...
SELECT ENAME, DNAME, LOC FROM DEPT T JOIN EMP E
ON T.DEPTNO = E.DEPTNO AND E.ENAME LIKE '%L%';
...
SELECT * FROM (SELECT TEAM_ID,  MAX(HEIGHT) FROM PLAYER GROUP BY TEAM_ID) T JOIN PLAYER P
